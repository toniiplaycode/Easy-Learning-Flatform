      ...
      <div className="lecture-course-current">
        {sectionAllLectureEachCourse?.length > 0 ? (
          sectionAllLectureEachCourse
            ?.slice() // Tạo bản sao nông để tránh sửa đổi dữ liệu gốc
            ...
            .map((section) => (
              <div className="lecture-course-item" key={section.id}>
                ...
                {section?.Lectures?.slice() // Tạo bản sao nông để tránh sửa đổi dữ liệu gốc
                  .sort((a, b) => a.position - b.position) // Sắp xếp bài giảng theo position
                  .map((lecture) => {
